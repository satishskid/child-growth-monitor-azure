...
class WHOStandards:
        ...
        try:
            ...
            if data_dir.exists():
                ...
                standard_files = {
                    ...
                    "head_circumference_for_age": data_dir
    ...
    def calculate_metrics(
        self, height_cm: float, weight_kg: float, age_months: int, gender: str
    ) -> Dict:
        """
        ...
        try:
            ...
            # Height-for-age
            height_metrics = self._calculate_height_for_age(
                height_cm, age_months, gender
            )
            metrics.update(height_metrics)

            # Weight-for-age
            weight_metrics = self._calculate_weight_for_age(
                weight_kg, age_months, gender
            )
            metrics.update(weight_metrics)

            # Weight-for-height
            wfh_metrics = self._calculate_weight_for_height(
                weight_kg, height_cm, gender
            )
    ...
    def _calculate_height_for_age(
        self, height_cm: float, age_months: int, gender: str
    ) -> Dict:
        """Calculate height-for-age percentile and z-score."""
        try:
            if self.standards_loaded and "height_for_age" in self.growth_tables:
                # Use actual WHO data
                return self._lookup_who_table(
                    "height_for_age", height_cm, age_months, gender
                )
            else:
                # Use approximation
    ...
    def _calculate_weight_for_age(
        self, weight_kg: float, age_months: int, gender: str
    ) -> Dict:
        ...
        try:
            if self.standards_loaded and "weight_for_age" in self.growth_tables:
                # Use actual WHO data
                return self._lookup_who_table(
                    "weight_for_age", weight_kg, age_months, gender
                )
            else:
                # Use approximation
    ...
    def _calculate_weight_for_height(
        self, weight_kg: float, height_cm: float, gender: str
    ) -> Dict:
    ...
    def _approximate_height_for_age(
        self, height_cm: float, age_months: int, gender: str
    ) -> Dict:
        ...
        try:
            ...
            monthly_increment = params["monthly_increment"] * (
                1 - age_months * 0.005
            )  # Decreases with age
    ...
    def _approximate_weight_for_age(
        self, weight_kg: float, age_months: int, gender: str
    ) -> Dict:
    ...
    def _lookup_who_table(
        self, table_name: str, measurement: float, age_months: int, gender: str
    ) -> Dict:
    ...
    def generate_growth_recommendations(
        self, metrics: Dict, age_months: int
    ) -> List[str]:
        ...
        try:
            ...
            if height_z < -2:
                recommendations.append(
                    "Child shows signs of stunting. Ensure adequate nutrition and monitor growth closely."
                )
                if height_z < -3:
                    recommendations.append(
                        "Severe stunting detected. Seek immediate nutritional intervention."
                    )
            ...
            if weight_z < -2:
                recommendations.append(
                    "Child is underweight. Increase caloric intake and protein-rich foods."
                )
                if weight_z < -3:
                    recommendations.append(
                        "Severe underweight. Immediate medical attention required."
                    )
            ...
            if wfh_z < -2:
                recommendations.append(
                    "Child shows signs of wasting. Focus on nutrient-dense foods."
                )
                if wfh_z < -3:
                    recommendations.append(
                        "Severe acute malnutrition. Emergency treatment needed."
                    )
            elif wfh_z > 2:
                recommendations.append(
                    "Child may be overweight. Ensure balanced nutrition and physical activity."
                )

            # Age-specific recommendations
            if age_months < 6:
                recommendations.append(
                    "Exclusive breastfeeding recommended until 6 months."
                )
            elif age_months < 24:
                recommendations.append(
            ...
            if not recommendations:
                recommendations.append(
                    "Child's growth appears normal. Continue regular monitoring."
                )
                recommendations.append(
                    "Maintain balanced nutrition and regular health check-ups."
                )
