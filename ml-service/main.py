...
class PoseKeypoints(BaseModel):
    """3D pose keypoints."""
    keypoints: List[List[float]] = Field(..., description="3D keypoints [x, y, z, confidence]")
    ...
    try:
        ...
        for scan_type, video_file in videos.items():
            if video_file is not None:
                ...
                # Process video
                scan_metadata = metadata.copy() if metadata else ScanMetadata(
                    child_age_months=24,
                    child_gender="male",
                    scan_type=scan_type
                )
...
def combine_multi_scan_results(
    scan_results: Dict[str, PredictionResult], 
    metadata: ScanMetadata
) -> PredictionResult:
    ...
    for measurement_type in ['height', 'weight', 'arm_circumference', 'head_circumference']:
        ...
        combined_measurements[measurement_type] = {
            ...
            'confidence': min(weighted_confidence * 1.1, 1.0)  # Slight boost for multi-scan
        }
...
def assess_nutritional_status(who_metrics: Dict) -> NutritionalAssessment:
    ...
    severe_conditions = [stunting_status, wasting_status, underweight_status].count('severe')
    moderate_conditions = [stunting_status, wasting_status, underweight_status].count('moderate')
    
    if severe_conditions > 0:
        overall_risk = 'critical'
    elif moderate_conditions > 1:
        overall_risk = 'high'
    elif moderate_conditions > 0 or [stunting_status, wasting_status, underweight_status].count('mild') > 1:
        overall_risk = 'medium'
    else:
        overall_risk = 'low'
    ...
    if stunting_status in ['moderate', 'severe']:
        recommendations.append("Immediate nutritional intervention required for stunting")
