...
class PoseEstimator:
    ...
    def estimate_pose(self, image: np.ndarray) -> Dict:
        ...
        try:
            ...
            quality_score = self._calculate_quality_score(
                visibility_scores, landmarks_2d
            )
    ...
    def _calculate_quality_score(
        self, visibility_scores: List[float], landmarks_2d: List[List[float]]
    ) -> float:
    ...
    def _format_keypoints(
        self, landmarks_2d: List[List[float]], landmarks_3d: List[List[float]]
    ) -> List[List[float]]:
        ...
        for i, landmark_2d in enumerate(landmarks_2d):
            if i < len(landmarks_3d):
                ...
                keypoint = [
                    ...
                    (
                        landmarks_3d[i][2] if landmarks_3d[i][2] != 0 else 0.0
                    ),  # z (normalized)
                    landmark_2d[2],  # confidence
                ]
            else:
                # 2D only
                keypoint = [landmark_2d[0], landmark_2d[1], 0.0, landmark_2d[2]]
